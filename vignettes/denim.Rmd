...
* `mathexpr(expression)`: User-defined **mathematical expression**, the expression can be put inside `""` or not, e.g `mathexpr(beta * S * I / N)` or `mathexpr("beta * S * I / N")` are both acceptable
...
```{r, fig.width = 6}
transitions <- list(
  "S -> I" = mathexpr(beta * S * (I + IV) / N),
  "S -> V" = constant(2),
  "0.1 * I -> D" = lognormal(2, 0.5),
  "0.9 * I -> R" = gamma(3, 2),
  "V -> IV" = mathexpr(0.1 * beta * V * (I + IV) / N),
  "IV -> R" = exponential(2)
)
