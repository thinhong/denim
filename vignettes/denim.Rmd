...
For a given length of stay distribution, we identify the maximum length of stay using its cumulative distribution function. Because cumulative distribution function is asymptotic to 1 and never equal to 1, we need to set a value that is acceptable to be rounded to 1. If we want a cumulative probability of 0.999 to be rounded as 1, we set the error tolerance threshold as `1 - 0.999 = 0.001` (specified by the argument `errorTolerance = 0.001`), .

## 3. Waiting time distribution

Current available distribution in this package including:

* `exponential(rate)`: Discrete **exponential distribution** with parameter `rate`

* `gamma(scale, shape)`: Discrete **gamma distribution** with parameters `scale` and `shape`

* `weibull(scale, shape)`: Discrete **Weibull distribution** with parameters `scale` and `shape`

* `lognormal(mu, sigma)`: Discrete **log-normal distribution** with parameters `mu` and `sigma`
...
* `constant(x)`: Every time step a **fixed number of individuals** of the left compartment transit to the right compartment, e.g `"S -> V" = constant(50)` means 50 people got vaccinated per day

* `transitionProb(transitionProb)`: Every time step a **fixed percentage** of the left compartment transit to the right compartment, this is also a convenient way to define $R_t - R_{t-1} = \gamma I$ which we can input `"I -> R" = transitionProb(gamma)`
...
* `multinomial(probabilities)`: A convenient way to define **several probabilities** of a compartment transit to many compartments, may or may not in a time step. For example, `"V -> VA, VS, VH" = multinomial(0.6, 0.3, 0.1)` means 60% of `V` will become `VA`, 30% become `VS` and 10% become `VH`. If we continue to define the length of stay distribution for these transitions e.g `"V -> VA" = gamma(3, 2)`, probabilities defined with `multinomial()` is not the percentage of the left compartment transit in each time step, but the percentage of individuals move to `VA` at the equilibrium state. If we do not define further length of stay distribution, `"V -> VA, VS, VH" = multinomial(0.6, 0.3, 0.1)` is the percentage of `V` transit to the right compartments per time step similar to a `transitionProb()` function. See more detailed explanations in the ***Multiple transitions from a compartment*** section.
...
DiagrammeR::grViz("digraph {
  ...
  S -> I [label = '&#946;SI/N']
  S -> V [label = 'constant(5)']
  I -> R [label = '0.9 -> gamma(3, 2)']
  I -> D [label = '0.1 -> lognormal(2, 0.5)']
  }",
...
DiagrammeR::grViz("digraph {
  ...
  S -> I [label = '&#946;S(I + IV)/N']
  S -> V [label = 'constant(2)']
  I -> D [label = '0.1 -> lognormal(2, 0.5)']
  I -> R [label = '0.9 -> gamma(3, 2)']
  V -> IV [label = '0.1 * &#946;S(I + IV)/N']
  IV -> R [label = 'exponential(2)']
  }",
...
```{r, fig.width = 6}
transitions <- list(
  "S -> I" = mathExpression(beta * S * (I + IV) / N),
  "S -> V" = constant(2),
  "0.1 * I -> D" = lognormal(2, 0.5),
  "0.9 * I -> R" = gamma(3, 2),
  "V -> IV" = mathExpression(0.1 * beta * V * (I + IV) / N),
  "IV -> R" = exponential(2)
)
