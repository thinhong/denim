...
<summary>Version specifications</summary>
...
# compute min, max, variance
# lapply(cached_runtime, function(x) c(min = min(x), max = max(x), var = var(x)))
```
...
![](./images/denim_website-benchmark-mod.png){width="700"}
...
<summary>Dependency version specifications</summary>
...
# Create the plot
runtime_plot <- ggplot(plot_df, aes(x = run, y = time, color = method)) +
...
It is worth noting that runtime for denim is also dependent on duration of time step (`timeStep` parameter for `sim`). This is discussed in details in the next section.

------------------------------------------------------------------------

# Denim performance details

## 1. Time and space complexity

In terms of big-O notation, denim has a time complexity of $O(T S^2 n)$ and space complexity of $O(T S + S^2n)$. Where:

-   $T$ is the total simulated time steps

-   $S$ is the number of states/compartments

-   $n$ is the maximum length of the sub-compartment chain

Under the assumption that individuals in a compartment can transition to every other compartment.

However, under most realistic use cases, individuals can only transfer to a limited number of states in which case the number of transitions denoted $m$ is $m < S^2$. The time complexity would then be $O (T m n)$ and space complexity is $O(T S + mn)$.

## 2. Impact of timeStep in denim

### 2.1. Run time and memory scaling

A change in values for `timeStep` can directly change the value for $T$ and $n$ in the big-O notation, which directly impact the run time and memory allocation of the algorithm.

The following plot demonstrates how run time and memory changes as value for `timeStep` changes, using the same model for benchmarking. The values for `timeStep` being evaluated are `[0.01, 0.02, 0.05, 0.1, 0.25, 0.5, 1]`.

```{r echo=FALSE}
timeStepDurations <- c(0.01, 0.02, 0.05, 0.1, 0.25, 0.5, 1)
benchmark_out <- lapply(timeStepDurations, 
     \(x){
       bench_out <- bench::mark(
...
runtime_scaling <- sapply(benchmark_out, \(bm){
  bm$median
})

memory_scaling <- sapply(benchmark_out, \(bm){
  as.numeric(bm$mem_alloc)/(1024)
})
```
...
```{r echo=FALSE, warning=FALSE}
data.frame(
  timeStepDur = timeStepDurations,
  runtime_scaling = runtime_scaling
) %>% 
  ggplot(aes(x = timeStepDurations, y = runtime_scaling)) +
    geom_point(color = "cornflowerblue", shape = 1, size=2) +
    geom_line(color = "cornflowerblue") +
    geom_vline(xintercept = timeStep,
               color = "red", linetype = "dashed", linewidth = 0.5) +
    geom_vline(xintercept = 1,
               color = "blueviolet", linetype = "dashed", linewidth = 0.5) +
    annotate("text",
             x = timeStep + 0.01,
             y = max(runtime_scaling),
             label = "timeStep for benchmarking",
             color = "red",
             hjust = 0) +
    annotate("text",
             x = 1 - 0.01,
             y = max(runtime_scaling), 
             label = "default timeStep",
             color = "blueviolet",
           hjust = 1) +
    labs(
      x = "Time step duration",
      y = "Median run time (in seconds)"
    ) +
    theme_bw()

# ggsave("../manuscript_plots/denim_runtime_scale_timeStep.png",
...
data.frame(
  ...
  ggplot(aes(x = timeStepDurations, y = memory_scaling)) +
    geom_point(color = "cornflowerblue", shape = 1, size=2) +
    geom_line(color = "cornflowerblue") +
    geom_vline(xintercept = timeStep,
               color = "red", linetype = "dashed", linewidth = 0.5) +
    ...
    labs(
      x = "Time step duration",
      y = "Memory allocated (in KBs)",
      caption = "*Memory allocations are tracked by Rprofmem, which records only allocations made through R’s memory allocator"
    ) +
    theme_bw()

# ggsave("../manuscript_plots/denim_memory_scale_timeStep.png",
#         width = 8, height = 5, dpi = 300)
```
...
The following plots demonstrate how precision is compromised as we increase the duration for time step (with output from `deSolve` used as baseline). The values for `timeStep` being evaluated are `[0.01, 0.1, 0.5, 1]`.
...
## 3. Math expression vs `d_*()` function

In scenarios where a compartmental model can be formulated using ODEs, `denim` provides 2 ways for users to define transitions between compartments:

-   Either using `d_*()` functions (`d_exponential()` or `d_gamma()`)

-   Or using the math expression

This section demonstrates the run time and numerical accuracy of these 2 approaches as `timeStep` varies, using results from `deSolve` as the baseline. For this demonstration, we use the basic SIR model.

```{r echo=FALSE}
# ---- Helper functions -----
# Function to filter the result at the time step of interest
# The input desolve_bm, and denim_bms are tibble of benchmark results returned by bench::mark
generate_plot_data <- function(desolve_bm, denim_bms, plot_timeStep = 0.01){
  denim_bms <- denim_bms %>% 
      filter(timeStep == plot_timeStep)
  
  # print(dim(denim_bms))
  # ---- reshape output to create pair-wise comparison plot ---
  bind_rows(
      map2_dfr(denim_bms$approach, denim_bms$result, \(approach, df){
        df %>% mutate(
          approach = approach
        ) %>% 
          as.data.frame()
      ...
      desolve_bm$result[[1]] %>% 
        as.data.frame() %>%  
        mutate(approach = "deSolve") %>% 
        rename(
          Time=time
        )
    ) %>% 
    select(Time, S, I, R, approach) %>% 
    pivot_longer(cols = c("S", "I", "R"), values_to = "pop", names_to = "comp") %>% 
    mutate(prop = pop / 1e3)
}

# Function to plot the result at the time step of interest
plot_pairwise2 <- function(df, plot_timeStep=0.01){
  reshaped_baseline <- df %>% 
    filter(approach == "deSolve") %>% 
    select(-approach) %>% 
    mutate(baseline = prop)
  ...
  reshaped_baseline %>% 
    ...
    scale_color_manual(
      values = c(
        "denim mathExpression" = "cornflowerblue",
        "denim d_* func" = "red"
      )
    ) +
    theme(legend.title = element_blank()) +
    ...
    labs(
      title = paste0("Pairwise plot at dt=", plot_timeStep),
      x = "deSolve (baseline)",
      y = "denim"
      # caption = "*Data points closer to the dashed line indicate better alignment between the baseline and denim outputs."
    )
...
```{=html}
<details>
  <summary>Model definition and configuration</summary>
```
The basic SIR model can be formulated using the following system of ODEs.
...
```{r echo=FALSE}
# ---- First, get the benchmark for deSolve (the baseline) ----
deSolve_bm <- bench::mark({
  ode(y = initialValues, times = times, parms = parameters, func = desolve_sir)
})

# ---- Then generate benchmark denim at different timesteps -----
timeStepDurations2 <- c(0.01, 0.02, 0.05, 0.1, 0.25, 0.5, 1)

# --- Benchmark the model using d_exponential() function ----
denim_dfunc_scaling <- map_dfr(timeStepDurations2, 
     \(x){
       bench_out <- bench::mark(
         sim(transitions = denim_dfunc,
                   initialValues = initialValues,
                   parameters = parameters,
                   simulationDuration = sim_duration, timeStep = x)
       )
       bench_out %>% 
         mutate(
           timeStep = x,
           approach = "denim d_* func",
           .before = 1
         )
     }
   )

# --- Benchmark the model using the math expression ----
denim_ode_scaling <- map_dfr(timeStepDurations2, 
     \(x){
       bench_out <- bench::mark(
         sim(transitions = denim_ode,
                   initialValues = initialValues,
                   parameters = parameters,
                   simulationDuration = sim_duration, timeStep = x)
       )
...
The plot below summarizes the run time and accuracy at different `timeStep` (denoted dt), comparing the use of `d_exponential()` vs math expression for `I->R` transition.

```{r echo=FALSE, fig.width=11.5, fig.height=6, fig.dpi=300}
# ---- Generate the individual plots ---- 
# First generate the plot for run time scaling using the 2 approaches
runtime_scaling_plot <- bind_rows(
  denim_dfunc_scaling,
  denim_ode_scaling
) %>% 
  mutate(
  ...
  ggplot() +
    geom_point(
      aes(x = timeStep, y = median_time, color = approach), 
      shape = 1, size=2) +
    geom_line(
      aes(x = timeStep, y = median_time, color = approach)) +
    geom_hline(yintercept = deSolve_bm$median %>% as.numeric(),
               color = "black", linetype = "dashed", linewidth = 0.5) +
    annotate("text",
             x = 0.1,
             y = deSolve_bm$median %>% as.numeric() + 0.0005, 
             label = "deSolve runtime",
             color = "black",
           vjust = 0) +
    scale_color_manual(
      values = c(
...
# then generate the plots at the representitive time steps: 0.01, 0.5 and 1
pair_001 <- generate_plot_data(
  desolve_bm = deSolve_bm,
  denim_bms = bind_rows(
    denim_dfunc_scaling,
    denim_ode_scaling
  ),
  ...
  plot_pairwise2(plot_timeStep = 0.01)

pair_05 <- generate_plot_data(
  desolve_bm = deSolve_bm,
  denim_bms = bind_rows(
    denim_dfunc_scaling,
    denim_ode_scaling
  ...
  plot_pairwise2(plot_timeStep = 0.5)

pair_1 <- generate_plot_data(
  desolve_bm = deSolve_bm,
  denim_bms = bind_rows(
    denim_dfunc_scaling,
    denim_ode_scaling
  ...
  plot_pairwise2(plot_timeStep = 1)

# --- Finally, combine all the plots ------ 
library(patchwork)
(
  runtime_scaling_plot + 
    geom_vline(
      data = tibble::tibble(
      x = c(0.01, 0.5, 1)
    ),
...
# Save figure
# ggsave(
#    "../manuscript_plots/dfunc_vs_math.png",
#    width = 11.5, height = 6, dpi = 300)
```

The plot for run time scaling (upper figure) indicates that using `d_exponential()` results in slightly faster run times as `timeStep` increases. This suggests that, for the current model, the overhead from parsing math expressions exceeds the time for iterating over sub-compartments.

As we\'ve known, run time decreases as dt increases, and at dt = 1 `denim` approaches the performance of `deSolve`. However, this also reduces accuracy, as suggested by the pairwise plot.

## 4. Performance in more complex model

To assess the performance of `denim` as the model grows more complex, we measure the run-time and memory allocation as more transitions are added to the model.

To do that, we consider the following sequential compartmental model where $S$ indicates the number of compartments

![](images/denim_website-runtime-scaling-mod.png){width="654"}

General settings:

-   `simulationDuration=2000`

-   `timeStep=0.01` (i.e., simulate the model for 200000 time steps)

-   The duration for any transition $X_i \rightarrow X_{i+1}$ is set to follow a log-normal distribution with $\mu = 2.2$ and $\sigma = 0.4$

    -   Maximal dwell time is \~13 time units (length of each sub-compartment chain is 1300)

Under the given setup, the number of transitions $m$ is simply $S-1$

We will test the run time and memory allocation for model with `[3, 5, 25, 50, 100]` compartments (i.e., `[2, 4, 24, 49, 99]` transitions under this settings).

***Run time***

```{r echo=FALSE}
cached_time_scaling <- if (file.exists("../supplements/cached_time_scaling.rds")) {
  readRDS("../supplements/cached_time_scaling.rds")
} else {
  NULL
}

compartment_no <- c(3, 5, 25, 50, 100)
parameters <- c(mu = 2.2, sigma = 0.4)
timeStep <- 0.01
simulationDuration <- 2000

runtime_scaling_out <- if(is.null(cached_time_scaling)){
  map_dfr(compartment_no, \(no){
    # build model with given number of compartments
    mod <- lapply(1:(no-1), \(curr_comp){
      d_lognormal(mu = "mu", sigma = "sigma")
    })
    names(mod) <- paste0("X", 1:(no-1) , "->X", 1:(no-1) + 1)
    
    # set the initial values
    init_vals <- rep(0, no)
    names(init_vals) <- paste0("X", 1:no)
    init_vals[1] <- 1e6
...
```{r echo=FALSE}
runtime_scaling_out %>% 
  ggplot(aes(x = no_comp, y = median)) +
    geom_line(color = "cornflowerblue") +
    geom_point(color = "cornflowerblue", shape = 1, size=2) +
    labs(
      title = "Run time at different number of compartments",
      x = "Number of compartments (S)",
      y = "Run time (in seconds)"
    ) +
    theme_bw()

# ggsave("../manuscript_plots/denim_runtime_scale_comp.png",
#        width = 8, height = 5, dpi = 300)
```
...
runtime_scaling_out %>% 
  mutate(
    mem_alloc_mb = as.numeric(mem_alloc)/(1000**2) 
  ) %>% 
  ggplot(aes(x = no_comp, y = mem_alloc_mb)) +
    geom_line(color = "cornflowerblue") +
    geom_point(color = "cornflowerblue", shape = 1, size=2) +
    labs(
      title = "Memory allocated at different number of compartments",
      x = "Number of compartments (S)",
      y = "Memory allocated by R & RCpp (MBs)",
      caption = "*Memory allocations are tracked by Rprofmem, which records only allocations made through R’s memory allocator"
    ) +
    theme_bw()

# ggsave("../manuscript_plots/denim_memory_scale_comp.png",
#        width = 8, height = 5, dpi = 300)
```
