...
* `gamma(scale, shape)`: Gamma distribution
* `weibull(scale, shape)`: Weibull distribution
...
* `constant(x)`: A fixed number of individuals moving per time step, e.g `"S -> V" = constant(50)` means 50 people got vaccinated per day
* `transitionProb(transitionProb)`: Every time step a fixed percentage of the left compartment transit to the right compartment, this is the conventional $R_t - R_{t-1} = \gamma I$ which we input by `"I -> R" = transitionProb(gamma)`
* `nonparametric(waitingTimes...)`: A vector of values, could be numbers, percentages, density of the length of stay based on real data
